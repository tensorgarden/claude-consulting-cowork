import { activities, averageConfidence, clients, failingIntegrationCount, healthyClientCount, heroMetrics, integrations, playbooks, readinessGates, validationReviews, workspaceAccessReviews } from "@/lib/demo-data";
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto flex max-w-7xl flex-col gap-8 px-6 py-8 lg:px-10">
        ...
        <Card>
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.25em] text-emerald-300">Access and action scope</p>
              <h2 className="mt-2 text-2xl font-bold text-white">What Claude can see and do before review</h2>
              <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">
                Buyers evaluating enterprise agents need a plain-English manifest of connected data, allowed drafting work, and actions blocked until a named human approves them.
              </p>
            </div>
            <Badge tone="emerald">{workspaceAccessReviews.length} access reviews</Badge>
          </div>
          <div className="mt-6 grid gap-4 lg:grid-cols-3">
            {workspaceAccessReviews.map((scope) => {
              ...
              return (
                <div key={scope.id} className="rounded-2xl border border-white/10 bg-slate-900/70 p-4">
                  ...
                  <p className="mt-4 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Data Claude can see</p>
                  <ul className="mt-3 space-y-2 text-sm text-slate-300">
                    {scope.dataScope.map((source) => (
                      <li key={`${scope.id}-${source}`} className="rounded-xl border border-sky-400/20 bg-sky-400/10 px-3 py-2">{source}</li>
                    ))}
                  </ul>
                  <p className="mt-4 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Blocked until approval</p>
                  <ul className="mt-3 space-y-2 text-sm text-slate-300">
                    {scope.blockedActions.map((action) => (
